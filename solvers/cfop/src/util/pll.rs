...
use super::oll::oll_solved;

pub fn pll_solved(state: &CubeState) -> bool {
    if !oll_solved(state) {
        return false;
    }
    if state.faces[1][1] != state.faces[1][0] || state.faces[1][1] != state.faces[1][2] {
        return false;
    }
    if state.faces[2][1] != state.faces[2][0] || state.faces[2][1] != state.faces[2][2] {
        return false;
    }
    if state.faces[3][1] != state.faces[3][0] || state.faces[3][1] != state.faces[3][2] {
        return false;
    }
    if state.faces[4][1] != state.faces[4][0] || state.faces[4][1] != state.faces[4][2] {
        return false;
    }
    true
}
...
pub const PLL_ALGORITHMS: &[&[CubeMove]] = &[
    ...
    // PLL E
    &[
        R2, U, FPrime, RPrime, U, R, UPrime, RPrime, U, R, UPrime, RPrime, U, R, UPrime, F, UPrime,
        R2,
    ],
    // PLL F
    &[
    ...
    // PLL Na
    &[
        FPrime, R, U, RPrime, UPrime, RPrime, F, R2, F, UPrime, RPrime, UPrime, R, U, FPrime,
        RPrime,
    ],
    // PLL Nb
    &[
    ...
    // PLL Y
    &[
        F, R, UPrime, RPrime, UPrime, R, U, RPrime, FPrime, R, U, RPrime, UPrime, RPrime, F, R,
        FPrime,
    ],
    // PLL Z
    &[
            ...
            for &scramble_moves_2 in PLL_ALGORITHMS {
                ...
                match result {
                    Ok(solutions) => {
                        ...
                        assert!(pll_solved(solved_state), "PLL solution did not solve PLL");
                        let mut test_state = state.clone();
                        for &mv in solution_moves {
                            test_state.execute_move(mv);
                        }
                        assert!(
                            pll_solved(&test_state),
                            "Applying PLL solution moves did not result in PLL solved state"
                        );
                    }
                    Err(e) => panic!(
                        "Failed to solve PLL where scramble is {:#?} {:#?}: {}",
                        scramble_moves_1, scramble_moves_2, e
                    ),
                }
            }
        }
